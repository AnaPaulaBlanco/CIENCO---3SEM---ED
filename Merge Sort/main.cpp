...
void merge(int vetor[TAM], int indiceEsquerdo, int meio, int indiceDireito)
{
    ...
    //passaos elementos do vetor principal para o primeiro vetor auxiliar(esquerda)
    for(i = 0; i < n1; i++)
    {
    ...
    // sefaltarem alguns elementos do array direito, passa eles para o array principal
    while(j < n2)
    {
        vetor[k] = vetorDireito[j];
        j++;
...
void mergeSort(int vetor[TAM], int indiceEsquerdo, int indiceDireito)
{
    if(indiceEsquerdo < indiceDireito)
    {
        ...
        //une os dois sbarrays que foram criados
